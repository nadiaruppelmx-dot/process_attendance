# -*- coding: utf-8 -*-
"""process_attendance.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1t_IV3OAeMyAzC4DbwI0QmGeuRn0Obres
"""
...
if __name__ == "__main__":
    ...
    if args.file:
        # Check if the provided file is an Excel file
        if not (args.file.lower().endswith(".xlsx") or args.file.lower().endswith(".xls")):
            print(f"⚠️  El archivo '{args.file}' no parece ser un archivo Excel válido.")
            print(f"   Asegúrate de que la extensión sea .xlsx o .xls.")
            exit(1)
        files = [args.file]
    else:
...
sin_salida_otros_empleados = df_d[(df_d['sin_salida'] == 1) & (df_d['empleado'] != 'JACOBO JUAREZ CORDOBA')]

if not sin_salida_otros_empleados.empty:
    print(f"\n⚠️ Turnos SIN salida registrada para otros empleados ({len(sin_salida_otros_empleados)}):\n")
    for _, r in sin_salida_otros_empleados.iterrows():
        print(f"   {r['empleado']}  {r['fecha']}  entrada: {r['hora_entrada']}")
else:
    print("\nNo se encontraron otros empleados con turnos sin salida registrada.")

import os

# Set Colab-friendly paths for the script's global variables
RAW_DIR = "/content"  # The input Excel file is in /content
PROCESSED_DIR = "/content/processed"
DB_PATH = "/content/attendance.db"

os.makedirs(PROCESSED_DIR, exist_ok=True)

# Call the run function with the specific file
print(f"Processing file: /content/Semana 1.xlsx")
df_d, df_i, df_s = run(["/content/Semana 1.xlsx"])